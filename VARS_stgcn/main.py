...
from dataset import MultiViewDataset, SkeletonDataset
...
_ALL_MODELS = _TORCHVISION_MODELS | set(HF_VIDEOMAE_REGISTRY.keys()) | {"stgcn"}
...
def checkArguments(args):
    ...
    if args.pre_model == "stgcn":
        if args.fusion_mode:
            raise ValueError("--fusion_mode is not supported with --pre_model stgcn")
        if not args.skeleton_hdf5_root:
            raise ValueError("--skeleton_hdf5_root must be set when --pre_model stgcn")
        if args.skeleton_norm not in ("Yes", "No"):
            raise ValueError("--skeleton_norm must be 'Yes' or 'No'")
        if args.skeleton_in_channels < 2:
            raise ValueError("--skeleton_in_channels must be >= 2")
        if args.skeleton_num_joints < 2:
            raise ValueError("--skeleton_num_joints must be >= 2")
...
def main(args):
    ...
    # --- augmentation ---
    if data_aug == "Yes" and not use_skeleton:
    ...
    # --- datasets & loaders ---
    dataset_cls = SkeletonDataset if use_skeleton else MultiViewDataset
    dataset_kwargs = dict(
        path=path,
        start=start_frame,
        end=end_frame,
    ...
    if use_skeleton:
        frame_size = (
            tuple(args.skeleton_frame_size)
            if args.skeleton_frame_size is not None
            else None
        )
        dataset_kwargs.update(
            dict(
                skeleton_hdf5_root=args.skeleton_hdf5_root,
                coord_norm=args.skeleton_norm == "Yes",
                coord_channels=args.skeleton_in_channels,
    ...
    if only_evaluation == 0:
        dataset_Test = dataset_cls(**dataset_kwargs, split="Test", num_views=5)
        ...
        dataset_Chall = dataset_cls(**dataset_kwargs, split="Chall", num_views=5)
        ...
        dataset_Valid = dataset_cls(
            **dataset_kwargs, split="Valid", num_views=num_views
        )
        dataset_Test = dataset_cls(**dataset_kwargs, split="Test", num_views=num_views)
    ...
    if use_skeleton:
        stgcn_config = dict(
            num_joints=args.skeleton_num_joints,
            in_channels=args.skeleton_in_channels,
            layout=args.skeleton_layout,
            edge_importance=not args.stgcn_no_edge_importance,
    ...
    freeze_backbone = freeze_epoch >= 0
    if use_skeleton:
    ...
    if args.continue_training and path_to_model_weights != "":
        ...
        if freeze_backbone and epoch_start > freeze_epoch:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Root folder with Train/Valid/Test HDF5 skeleton files",
    )
    parser.add_argument(
        "--skeleton_norm",
        default="Yes",
        type=str,
        help="Normalize x/y coordinates to [-1, 1] (Yes/No)",
    )
    parser.add_argument(
        "--skeleton_in_channels",
        default=3,
        type=int,
        help="Number of coordinate channels (2 = x,y; 3 = x,y,conf)",
    )
    parser.add_argument(
        "--skeleton_num_joints",
        default=17,
        type=int,
        help="Number of joints per frame",
    )
    parser.add_argument(
        "--skeleton_layout",
        default="coco17",
        type=str,
        help="Skeleton layout (coco17, openpose18, or custom chain)",
    )
    parser.add_argument(
        "--skeleton_frame_size",
        nargs=2,
        ...
        help="Frame size for pixel-coordinate normalization",
    )
    ...
    parser.add_argument(
        ...
        default=0.1,
        type=float,
        help="Dropout used in ST-GCN temporal blocks",
    )
    parser.add_argument(
        "--stgcn_no_edge_importance",
        action="store_true",
        default=False,
        help="Disable per-layer edge-importance weights in ST-GCN",
    )
    ...
    # Infra
    parser.add_argument("--model_name", default="VARS_stgcn", type=str)
