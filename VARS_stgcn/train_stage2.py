...
def _run_loader(model, loader, fusion_mode: bool):
    ...
    for batch in tqdm(loader, leave=False):
        t_sev, t_act, t_contact, t_bodypart, t_ttp, t_handball, mvclips, action_ids = (
            batch
        )
    ...
    return (
        ...
        np.concatenate(sevs),
        np.concatenate(acts),
        np.concatenate(contacts),
        np.concatenate(bodyparts),
        np.concatenate(ttps),
        np.concatenate(handballs),
        ids,
    )
        ...
        torch.save(
            {
                ...
                "scheduler": scheduler.state_dict(),
                "uw": (
                    uncertainty_weighter.state_dict() if uncertainty_weighter else None
                ),
            },
...
def main(args):
    ...
    transforms_model = (
        ...
        if args.fusion_mode
            ...
            args.pre_model, R2Plus1D_18_Weights.KINETICS400_V1.transforms()
        )
    )
    ...
    if args.mode in ("train_heads", "full"):
        ...
        logging.info(
            f"Feature bank: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}"
        )

        # Inverse-frequency action weights from bank
        act_counts = np.bincount(train_ds.targets_act.numpy(), minlength=8).astype(
            np.float32
        )
