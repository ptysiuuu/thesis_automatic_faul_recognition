...
from ..utils.constants import ACTION_CLASSES, SEVERITY_CLASSES, PER_ACTION_SEVERITY_PRIOR
...
def run_full_sev_two_stage(
    backend, frames_per_view: List[List[Image.Image]],
    law12_ctx: str, medoid_cache: dict, severity_priors: dict, per_action_priors: dict,
    frames_per_view_count: int, action_hint: str,
    rag,
) -> Tuple[int, int, str]:
    """
    ...
    act_prompt = build_cos_action_prompt(
        ...
        mined_examples=all_text, selected_frame_info=sel_info)
    ...
    sev_prompt = build_full_frame_severity_prompt(
        n_views=n_views, law12_context=sev_law12,
        predicted_action=act_str, severity_examples=sev_examples_text,
...
def run_per_action_prior(
    backend, frames_per_view: List[List[Image.Image]],
    law12_ctx: str, per_action_priors: dict,
) -> Tuple[int, int, str]:
    """
...
def run_cos_full_sev(
    ...
    raw = (f"STAGE0 (CoS selection): {raw0}\n"
           f"Selected: {sel_info}\n"
           f"STAGE1 (action, key frames): {raw1}\n"
...
def run_flow_hard_neg(
    backend, frames_per_view: List[List[Image.Image]], 
    law12_ctx: str, medoid_cache: dict, severity_priors: dict,
    rag, retriever
) -> Tuple[int, int, str]:
    ...
    sev_prompt = build_targeted_retrieval_prompt(
        n_views=n_views, law12_context=sev_law12, 
        targeted_examples=hard_neg_text, predicted_action_hint=act_str
    )
    raw2 = backend.classify(frames_per_view, sev_prompt)
    sev_idx = parse_severity_only(raw2)
    
    return act_idx, sev_idx, f"STAGE1 (Flow Action): {raw1}\nSTAGE2 (Hard Neg Severity): {raw2}"
