...
def extract_keyframes_weighted(
    hdf5_file, action_key: str, clip_key: str, n_frames: int = 8
) -> List[Image.Image]:
    """
...
def _read_video_frames(video_path: str, indices: List[int]) -> List[Image.Image]:
    import cv2
...
def extract_keyframes_from_video(
    ...
    return _read_video_frames(video_path, indices)


def extract_keyframes_weighted_from_video(
    ...
    center = total // 2
    dense_start = max(0, center - total // 5)
    dense_end = min(total - 1, center + total // 5)
    ...
    early = np.linspace(0, dense_start, n_edge + 1, dtype=int)[:-1]
    dense = np.linspace(dense_start, dense_end, n_dense, dtype=int)
    late = np.linspace(dense_end, total - 1, n_edge + 1, dtype=int)[1:]

    indices = np.unique(np.concatenate([early, dense, late]))[:n_frames].tolist()
    return _read_video_frames(video_path, indices)
...
def _split_dir(split: str) -> str:
    ...
    if split_lower.startswith("train"):
        return "Train"
    if split_lower.startswith("valid") or split_lower.startswith("val"):
        return "Valid"
    if split_lower.startswith("test"):
        return "Test"
    if split_lower.startswith("chall"):
        return "Chall"
    return split.capitalize() if split else "Valid"


def extract_all_views_from_video(
    video_root: str,
    split: str,
    action_id: str,
    clips: list,
    ...
    extractor = (
        extract_keyframes_weighted_from_video
        if weighted
        else extract_keyframes_from_video
    )

    split_dir = _split_dir(split)
    action_dir = os.path.join(video_root, split_dir, f"action_{action_id}")

    frames_per_view = []
    for clip_raw in clips[:max_views]:
        clip_name = clip_raw if clip_raw.endswith(".mp4") else f"{clip_raw}.mp4"
        video_path = os.path.join(action_dir, clip_name)
        frames = extractor(video_path, n_frames=n_frames)
        if frames:
            frames_per_view.append(frames)
...
def extract_flow_keyframe(
    hdf5_file, action_key: str, clip_key: str, context_window: int = 1
) -> List[Image.Image]:
...
def extract_all_views_flow(
    hdf5_file, action_key: str, clips: list, max_views: int = 4
) -> List[List[Image.Image]]:
    frames_per_view = []
    for clip_raw in clips[:max_views]:
        clip_key = clip_raw.replace(".mp4", "")
