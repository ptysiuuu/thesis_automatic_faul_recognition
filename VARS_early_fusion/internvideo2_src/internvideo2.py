...
class Attention(nn.Module):
    ...
    def _flash_attn(self, x, key_padding_mask=None, need_weights=False):
        ...
        context = flash_attn_qkvpacked_func(
            qkv, dropout_p=dropout_val, casual=self.casual
        )
...
class InternVideo2(nn.Module):
    def __init__(
        ...
        if use_fused_rmsnorm and DropoutAddRMSNorm is not None:
...
if __name__ == "__main__":
    ...
    flops = FlopCountAnalysis(
        model, torch.rand(1, 3, num_frames, img_size, img_size).cuda().half()
    )
    s = time.time()
