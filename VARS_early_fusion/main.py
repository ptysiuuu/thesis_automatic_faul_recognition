...
def main(args):
    ...
    # --- logging ---
    os.makedirs(
        os.path.join("models", model_name, str(num_views), backbone_id,
                     str(LR), f"_B{batch_size}_F{number_of_frames}_G{gamma}_Step{step_size}"),
        exist_ok=True
    )
    best_model_path = os.path.join(
        "models", model_name, str(num_views), backbone_id,
        str(LR), f"_B{batch_size}_F{number_of_frames}_G{gamma}_Step{step_size}"
    )
    logging.basicConfig(
        level=logging.INFO,
    ...
    transforms_model = (
        ...
        if fusion_mode
        else _transform_map.get(pre_model, R2Plus1D_18_Weights.KINETICS400_V1.transforms())
    )

    # --- datasets & loaders ---
    dataset_kwargs = dict(
        path=path, start=start_frame, end=end_frame, fps=fps,
        transform_model=transforms_model, fusion_mode=fusion_mode,
    )
    ...
    if only_evaluation == 0:
        ...
        dataset_Valid = MultiViewDataset(**dataset_kwargs, split='Valid', num_views=num_views)
        dataset_Test  = MultiViewDataset(**dataset_kwargs, split='Test',  num_views=num_views)
...
if __name__ == '__main__':
    parser = ArgumentParser(description='VARS — early fusion + multi-view modes',
